...
2. The [Docker containers that NVIDIA supplies](https://ngc.nvidia.com/catalog/containers?orderBy=modifiedDESC&pageNumber=0&query=jetson&quickFilter=containers&filters=) do not bundle in most of the hardware-specific libraries needed to run them, but expect them to be provided by the underlying host OS, so be sure to include TensorRT ([note](https://github.com/madisongh/meta-tegra/wiki/L4T-R32.3.1-Notes#tensorrt-packaging-change)), CuDNN, and/or VisionWorks, if you expect to be running containers needing those packages.
