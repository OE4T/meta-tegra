...
DEPENDS = "glib-2.0 gstreamer1.0 gstreamer1.0-plugins-base gstreamer1.0-rtsp-server \
    tensorrt-core tensorrt-plugins libnvvpi1 libvisionworks json-glib tegra-libraries \
"
...
do_configure() {
    for feature in amqp kafka redis triton; do
...
def pkgconf_packages(d):
    pkgconf = bb.utils.filter('PACKAGECONFIG', 'amqp kafka redis triton', d).split()
    pn = d.getVar('PN')
    return ' '.join(['{}-{}'.format(pn, p) for p in pkgconf])

PKGCONF_PACKAGES = "${@pkgconf_packages(d)}"

PACKAGES =+ "${PN}-samples-data ${PN}-samples ${PN}-sources ${PN}-iothub-client ${PN}-azure-edge ${PKGCONF_PACKAGES}"
...
FILES_${PN}-azure-edge = "${DEEPSTREAM_PATH}/lib/libnvds_azure*"
FILES_${PN}-triton = "\
